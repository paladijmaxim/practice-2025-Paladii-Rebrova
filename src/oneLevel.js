const graph = {
    ...
    'empty2': ['corridor-6'],
    'stairs2': ['corridor-6'],
    'corridor-0': ['toilet', '2116', 'stairs1', '2110', 'corridor-1'],
    'corridor-1': ['corridor-0', '2115', '2110A', 'corridor-2'],
    'corridor-2': ['corridor-1', '2114', '2109', 'corridor-3'],
    'corridor-3': ['corridor-2', 'electrical', '2108', 'corridor-4'],
    'corridor-4': ['corridor-3', 'elevator', '2107', 'corridor-5'],
    'corridor-5': ['corridor-4', '2101', 'exit1', 'exit2', 'corridor-6'],
    'corridor-6': ['corridor-5', 'empty1', 'empty2', 'stairs2']
};
...
function buildEvacuationPath(start) {
    ...
    // Находим пути ко всем возможным выходам
    const exits = ['exit1', 'exit2', 'stairs1', 'stairs2'];
    exits.forEach(exit => {
        const path = findShortestPath(start, exit);
...
function showRouteInfo() {
    const exitName = {
        'exit1': 'Главный выход (левый)',
        'exit2': 'Главный выход (правый)',
        'stairs1': 'Лестница (левая)',
        'stairs2': 'Лестница (правая)'
    };
    
    const exit = exitName[currentPath[currentPath.length - 1]] || currentPath[currentPath.length - 1];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.room:not(.exit):not(.stairs)').forEach(room => {
